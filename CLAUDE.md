...
The Catalog context owns Products and their variants, components, and choice groups. Detailed entity model in [`design/catalog.md`](design/catalog.md); decisions and reasoning in [ADR-0005](decisions/0005-catalog-and-product-modeling.md) (base), [ADR-0023](decisions/0023-product-composition-and-choice-modeling.md) (four-mode discriminator; Composite and Configurable Products), and [ADR-0025](decisions/0025-optional-per-variant-sku-and-pattern-b-projection.md) (optional per-variant SKU; Pattern B as sole Variant-mode projection). At charter level:
...
* **Per-variant SKU** — `ProductVariant.sku` is **optional** (per [ADR-0025](decisions/0025-optional-per-variant-sku-and-pattern-b-projection.md)) and lives in a **single per-store SKU namespace** with `Product.sku`. A Variant's effective SKU is `variant.sku ?? parent.sku ?? null`. The Variant-vs-Standalone decision rule is no longer "do you need per-variant SKU?" — it's "do items share name / description / category / lifecycle?".
* **Variant-mode projection** — every Variant-mode Product projects to Beckn as `PLAIN` parent + sibling `VARIANT` Resources (Pattern B), one Resource per `ProductVariant`, regardless of whether any Variant carries an own SKU, price override, or media. Pattern A (`variantMatrix`) is explicitly out of scope in v1; may be added later as a pure Bridge optimization if wire bloat becomes a real concern.
