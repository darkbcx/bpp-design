...
- **Partially superseded by**: [ADR-0022](0022-onix-protocol-gateway.md) — direct CDS publishing from the Bridge is replaced; the Bridge now publishes to ONIX which forwards to CDS. Order state machine + Quote semantics + Bridge ↔ Order interface (in domain terms) survive unchanged.
